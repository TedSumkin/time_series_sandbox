# TODO — Pet Sandbox: Task × Model (2 недели)
...
## Неделя 1 — Каркас + 1 задача (Forecasting) + первые предохранители

### День 1: Скелет репо и “контракт интерфейсов”

- [ ] Создать репозиторий, структуру папок:
  - `sandbox/`
    - `tasks/`
    - `models/`
    - `baselines/`
  ...
  - `configs/`
  - `README.md`
- [ ] Зафиксировать “контракт интерфейсов” (в README или `sandbox/interfaces.md`):
  - Task:
  ...
  - Runner:
    - `fit(task, model, dataloaders, cfg) -> artifacts`
    - `evaluate(task, model, dataloader, cfg) -> metrics + optional preds`
- [ ] Принять формат batch (минимум для forecasting):
  - `batch = { "x": (B,T,F), "y": (B,H,F_out), "meta": {...} }` (или эквивалент)

### День 2: Runner v0 (train/eval, логи, чекпоинты, артефакты)
...
### День 3: Task v0 — Forecasting на простом датасете
...
### День 4: 2 baseline’а (контрольные точки)
...
### День 5: Предохранители v1 (минимум 5) + pytest каркас
...
## Неделя 2 — Расширение: 4 baseline’а + 10 предохранителей + 2-я задача

### День 8: Ещё 2 baseline’а
...
### День 9: Предохранители v2 (довести до 10)
...
### День 10: Оформить checks как pytest + удобные сообщения
...
### День 11: Вторая задача (минимальная) — на выбор
...
### День 12: Универсализация формата batch (без боли)
...
### День 13: Репродьюсибилити и “таблица результатов”
...
### День 14: README “как продукт” + финальная проверка
...
- [ ] Финальный критерий “готово Week 2”:
