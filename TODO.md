...
- [x] Реализовать стартовый датасет:
  - [x] Без нормализаций
...
Добавить ещё 5:
  1. [x] `determinism_check`: 2 запуска sanity-run с одним seed дают близкие результаты (допуск)
...
- [x] Перевести checks в `tests/`:
  - [x] позитивные тесты (всё проходит)
  - [x] негативные тесты (намеренно ломаем и ожидаем fail)
- [x] Сделать сообщения checks максимально диагностическими (“что именно не так”)
